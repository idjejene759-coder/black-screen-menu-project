...
# =============================================================================
# SQL ESCAPE HELPER (Simple Query Protocol — no %s params)
# =============================================================================

def esc(val):
    """Escape value for SQL Simple Query Protocol."""
    if val is None:
        return "NULL"
...
def sql_str(val):
    """Return escaped SQL string literal or NULL."""
    if val is None:
        return "NULL"
    return f"'{esc(val)}'"
...
def get_auth_token(cursor, token: str) -> Optional[dict]:
    ...
    cursor.execute(f"""
        ...
        WHERE token_hash = '{esc(token_hash)}'
    """)
...
def mark_token_used(cursor, token: str) -> bool:
    ...
    cursor.execute(f"""
        ...
        SET used = TRUE
        WHERE token_hash = '{esc(token_hash)}' AND used = FALSE
        RETURNING id
    """)
...
def find_user_by_telegram_id(cursor, telegram_id: str) -> Optional[dict]:
    ...
    cursor.execute(f"""
        ...
        FROM {schema}users
        WHERE telegram_id = '{esc(telegram_id)}'
    """)
...
def create_or_update_user(
    ...
    if existing:
        ...
        cursor.execute(f"""
            UPDATE {schema}users
            SET name = COALESCE({sql_str(display_name)}, name),
                avatar_url = COALESCE({sql_str(photo_url)}, avatar_url),
                last_login_at = NOW(),
                updated_at = NOW()
            WHERE telegram_id = '{esc(telegram_id)}'
        ...
        cursor.execute(f"""
            ...
            VALUES ('{esc(telegram_id)}', '{esc(display_name)}', {sql_str(photo_url)}, TRUE, '', NOW(), NOW(), NOW())
            RETURNING id, email, name, avatar_url, telegram_id, display_id, is_blocked, block_reason
        """)
...
def save_refresh_token(cursor, user_id: int, token_hash: str, expires_at: datetime) -> None:
    ...
    cursor.execute(f"""
        ...
        VALUES ({int(user_id)}, '{esc(token_hash)}', '{esc(expires_str)}')
    """)
...
def find_refresh_token(cursor, token_hash: str) -> Optional[dict]:
    ...
    cursor.execute(f"""
        ...
        WHERE token_hash = '{esc(token_hash)}' AND expires_at > NOW()
    """)
...
def delete_refresh_token(cursor, token_hash: str) -> None:
    ...
    schema = get_schema()
    cursor.execute(f"DELETE FROM {schema}refresh_tokens WHERE token_hash = '{esc(token_hash)}'")
